...
from PyQt5.QtWidgets import (
    ...
    QMenu, QFileDialog, QAction, QTextEdit,QDialog
)
...
"}")
        ...
        self.filesTable.setColumnWidth(0,195) 
    ...
    def upload_file(self):
        ...
        file_name, _ = QFileDialog.getOpenFileName(None, "Upload File", filter="MP3 files (*.mp3)")
        if file_name:
    ...
    def upload_folder(self):
        ...
        if folder_name:
            file_paths = [
                ...
                if os.path.isfile(os.path.join(folder_name, f)) and f.lower().endswith('.mp3')
            ]
            non_mp3_files = [
                f for f in os.listdir(folder_name)
                if os.path.isfile(os.path.join(folder_name, f)) and not f.lower().endswith('.mp3')
            ]
            if non_mp3_files:
                self.statusBar.append(f"Selected folder '{os.path.basename(folder_name)}' has non-MP3 files which will not be processed: {', '.join(non_mp3_files)}")
    ...
    def add_file_to_table(self, file_paths):
        ...
        # Remove the plus button row if it exists
        if row_position >= 0 and isinstance(self.table_widget.cellWidget(row_position, 0), QPushButton):
            self.filesTable.removeCellWidget(row_position, 0)
            self.filesTable.removeRow(row_position)

        # Add file names and sizes to the table
        for file_path in file_paths:
            ...
            self.filesTable.setItem(row_position, 1, QTableWidgetItem(f"{file_size} bytes"))

        # Add a new row with a plus symbol
        self.add_plus_symbol()
    ...
    def populate_table(self):
        ...
        data = pd.read_excel("LID_RESULT.xlsx")
        ...
        def insert_row(i):
            ...
            if i <= len(data) and  stop_insertion==False:
                ...
                timer = Timer(3.0, lambda: insert_row(i + 1))
                timer.start()

        # Start inserting the first row
        timer=Timer(5.0,lambda:insert_row(0))
        timers.append(timer)
        timer.start()

    def run_button_clicked(self):
        # Schedule population with 5 seconds delay
        timer = Timer(5.0, self.populate_table)
        timer.start()
    ...
    def save_button_clicked(self):
        ...
        if fileName:
            if self.save_to_csv(fileName, self.resultTable_2):
                ...
                MainWindow.statusBar().showMessage("File saved successfully", 2000)
            else:
